...
window.addEventListener("load", () => {
        ...
        switch (data.name) {
            ...
            case "Settings":
                ...
                Object.entries(data.value).forEach(([key, value]) => {
                    let setting = document.querySelector(`div.tab input[name=${key}]`);
                    if (setting) { if (setting.type === "checkbox") setting.checked = value === "true"; else setting.value = value; }
                });
    ...
    fe.ws.onopen = () => { fe.toast.fire({ icon: "success", title: fe.lang.con_suc, timer: 800 }); fe.ws.send(JSON.stringify({ name: "GetFiles", path: $("#settings_default_path").val() })); }
    ...
    $("div.tab").on("click", (a) => a.currentTarget === a.target ? a.currentTarget.classList.remove("active") : null);
    $("div.tab").on("click", "a.close", (a) => a.delegateTarget.classList.remove("active"));

    $("div.tab[data-tab=settings]").on("change", "input", (a) => {
        let { name, value, checked } = a.currentTarget;
        if (name === "performance") value = String(checked);
        else if (name === "defPath" && value.length === 0) value = "/";
        fe.ws.send(JSON.stringify({ name: "Settings", cname: name, value }));
    });
