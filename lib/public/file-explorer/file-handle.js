...
window.addEventListener("load", () => {
    ...
    /** Top Header */
    $(`${fe._} div.top-header`).on("click", "a.wforurl", GetFiles).on("click", ".address a[data-url]", GetFiles);
...
FileExplorer.GetFiles = function (data) {
    ...
    /** Address */
    top_header.find(".address").html(`<a data-url="/"><span>~</span></a>` + [...path].map((a, b, c) => `<a data-url="/${c.slice(0, b + 1).join("/")}"><span>${a}</span></a>`).join(" ") + `<input type="text" value="${fe.path}">`);
...
FileExplorer.PostUploadFile = async (file) => new Promise(res => {
    function sendChunk(file, offset = 0, path) {
        ...
        reader.onload = function (_e) {
            let base64Chunk = reader.result.split(",")[1];

            let _data = { fileName: file.name, chunk: base64Chunk, lastChunk: offset + chunkSize >= file.size };
            if (fe.ws.readyState === WebSocket.OPEN) fe.ws.send(JSON.stringify({ ..._data, name: "PostUpload", path: path + file.name }));
...
$(document).ready(async () => {
    await (() => new Promise(async (res) => {
        let map = await (await fetch(`/lang/lang.map.json`)).json();
        ...
        fe.lang = await (await fetch(`/lang/${priority[0]}.json`)).json();
